...
export const IntegrityFlags = () => {
  ...
  const fetchFlags = async (lvl: string) => {
    ...
    setError(null);
    try {
      const url = lvl === "ALL" ? `${API_BASE_URL}/integrity-flags` : `${API_BASE_URL}/integrity-flags?level=${lvl}`;
      const res = await Api.get<IntegrityFlag[]>(url);
      setFlags(res || []);
    } catch (err) {
      ...
      >
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Select value={level} onValueChange={(v) => setLevel(v as "ALL" | "HIGH" | "MEDIUM" | "LOW") }>
              ...
              <div>
                    <div className="flex items-center gap-3">
                      ...
                      <Badge className={`text-xs ${
                        f.level === "HIGH" ? "bg-red-200 text-red-800" :
                        f.level === "MEDIUM" ? "bg-yellow-200 text-yellow-800" :
                        f.level === "LOW" ? "bg-green-200 text-green-800" :
                        "bg-government-200 text-government-800"
                      }`}>{f.level}</Badge>
                    </div>
